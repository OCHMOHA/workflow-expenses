...
type DepenseRowDb = DepenseRow & {
  saisisseur?: { nom_complet: string | null } | null;
};
...
type StatusFilter = '' | 'brouillon' | 'soumise' | 'validee' | 'rejetee' | 'a_valider_n2';
...
async function getAllDepenses(): Promise<DepenseRow[]> {
  const { data, error } = await supabase
    .from('depenses')
    .select(
      'id, date_depense, categorie, montant_ttc, statut, saisisseur_id, ligne, sous_ligne, libelle, piece_justificative_url, mode_reglement, nom_beneficiaire_reglement, piece_reglement_url, reglee_at, reglee_par'
    )
  ...
  return data as DepenseRow[];
}

const CACHE_TTL_MS = 1000 * 30;

export default function ResponsableDepensesPage() {
  const router = useRouter();
  const [depenses, setDepenses] = React.useState<DepenseRow[]>([]);
  const [filter, setFilter] = React.useState<StatusFilter>('soumise');
  ...
  const openPieceJustificative = React.useCallback(async (d: DepenseRow) => {
    setPjError(null);
    setPjSignedUrl(null);
    ...
    if (error || !data?.signedUrl) {
      setPjError(error?.message ?? 'Unable to open attachment.');
      return;
    }
...
<html>
  ...
  const markTicketAsReglee = async (d: DepenseRow) => {
    ...
    const mode = (d.mode_reglement ?? '').toString().trim().toLowerCase();
    if (mode !== 'espèce' && mode !== 'espece') {
      setReglementError("Seuls les tickets en mode 'Espèce' peuvent être marqués comme réglés.");
      return;
    }
    ...
    try {
      ...
      if (d.saisisseur_id) {
        try {
          await createNotificationForDepense({
            depenseId: d.id,
  ...
  const selectedTvaLabel = React.useMemo(() => {
    if (!selected) return '-';
    const mtva = selected.montant_tva;
    const t = selected.tva;
    if (t == null) return '-';
    if (t === 0 && typeof mtva === 'number' && mtva > 0) return 'NC';
    return `${formatDzd(t)}%`;
  }, [selected]);

  const selectedMontantTva = React.useMemo(() => {
    if (!selected) return null;
    if (typeof selected.montant_tva === 'number') return selected.montant_tva;
    if (typeof selected.montant_ht === 'number' && typeof selected.tva === 'number') {
      return (selected.montant_ht * selected.tva) / 100;
    }
    return null;
  }, [selected]);
  ...
  const createNotificationForDepense = React.useCallback(
    async (args: {
      depenseId: string;
      userId: string;
  ...
  const notifyResponsablesN2 = React.useCallback(
    async (depenseId: string) => {
      const { data: profiles, error } = await supabase
        .from('profiles')
        .select('id')
        .eq('role', 'responsable niveau 2');
      ...
      const numero = `n°${depenseId.slice(0, 4).toUpperCase()}`;
      await Promise.all(
        ids.map((userId) =>
          createNotificationForDepense({
            depenseId,
            userId,
            type: 'DEPENSE_VALIDEE',
            title: 'Validation niveau 2 requise',
            body: `La dépense ${numero} nécessite une validation de niveau 2 (montant > 100 000 DA).`,
          })
        )
      );
    },
  ...
  const updateStatut = async (next: StatusFilter) => {
    if (!selected) return;
    const currentNormalized = normalizeStatut(selected.statut);
    if (currentNormalized && currentNormalized === next) return;
    const shouldEscalateToN2 = next === 'validee' && (selected.montant_ttc ?? 0) > 100000;
    const effectiveNext: StatusFilter = shouldEscalateToN2 ? 'a_valider_n2' : next;
    const dbValue = toDbStatut(effectiveNext);
    if (!dbValue) return;
    ...
    try {
      const { error } = await supabase
        ...
        .update({
          statut: dbValue,
          motif_rejet: dbValue === 'REJETEE' ? selected.motif_rejet ?? null : null,
          valideur_id: dbValue === 'VALIDEE' || dbValue === 'A_VALIDER_N2' ? currentUserId : null,
        })
        .eq('id', selected.id);
      ...
      updateStatutLocal(dbValue);
      setActionSuccess(
        dbValue === 'VALIDEE'
          ? 'Dépense validée.'
          : dbValue === 'REJETEE'
            ? 'Dépense rejetée.'
            : dbValue === 'A_VALIDER_N2'
              ? 'Soumise au niveau 2.'
            : 'Dépense soumise.'
      );

      if (dbValue === 'A_VALIDER_N2' && currentNormalized !== 'a_valider_n2') {
        try {
          await notifyResponsablesN2(selected.id);
        } catch (e: unknown) {
          setFetchError(e instanceof Error ? e.message : String(e));
        }
      }

      if (dbValue === 'VALIDEE' && currentNormalized !== 'validee') {
        const { data: depenseMeta, error: depenseMetaError } = await supabase
          .from('depenses')
          .select('saisisseur_id')
          .eq('id', selected.id)
          .maybeSingle();

        const saisisseurId = (depenseMeta as { saisisseur_id?: string | null } | null)
          ?.saisisseur_id;

        if (!depenseMetaError && saisisseurId) {
          try {
            await createNotificationForDepense({
              ...
              body: `Votre dépense n°${selected.id.slice(0, 4).toUpperCase()} a été validée.`,
            });
          } catch (e: unknown) {
            setFetchError(e instanceof Error ? e.message : String(e));
          }
        } else if (depenseMetaError) {
          setFetchError(depenseMetaError.message);
        }
      }
  ...
  const confirmReject = async () => {
    ...
    const { error } = await supabase
      .from('depenses')
      .update({ statut: 'REJETEE', motif_rejet: reason })
      .eq('id', selected.id);

    if (!error) {
      updateStatutLocal('REJETEE');
      setActionSuccess('Dépense rejetée.');
      if (selected.saisisseur_id && currentNormalized !== 'rejetee') {
        const numero = `n°${selected.id.slice(0, 4).toUpperCase()}`;
        const libelle = (selected.libelle ?? '').toString().trim();
        const libellePart = libelle ? ` dont le libellé est "${libelle}"` : '';
        await createNotificationForDepense({
          depenseId: selected.id,
          userId: selected.saisisseur_id,
          type: 'DEPENSE_REJETEE',
          title: 'Dépense rejetée',
          body: `Votre dépense ${numero}${libellePart} a été rejetée pour le motif suivant : ${reason}`,
        });
      }
      setRejectOpen(false);
      setRejectReason('');
  ...
  return (
    <>
                ...
                {filteredDepenses.length === 0 ? (
                  ...
                  filteredDepenses.map((d) => (
                    ...
                    >
                      ...
                      <TableCell sx={{ whiteSpace: 'nowrap' }}>
                        {d.saisisseur_nom ??
                          (d.saisisseur_id ? d.saisisseur_id.slice(0, 8) : '-')}
                      </TableCell>
              ...
              <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1.5 }}>
                <Box>
                  ...
                  <Typography variant="body1">
                    {selected?.id ? `n°${selected.id.slice(0, 4).toUpperCase()}` : '-'}
                  </Typography>
                </Box>
                ...
                <Box>
                  ...
                  <Typography variant="body1">
                    {selected?.montant_ht != null ? `${formatDzd(selected.montant_ht)} DZD` : '-'}
                  </Typography>
                </Box>

                <Box>
                  <Typography variant="subtitle2">TVA</Typography>
                  <Typography variant="body1">{selectedTvaLabel}</Typography>
                </Box>
                ...
                {(selected?.nom_beneficiaire || null) && (
                  <Box>
                    <Typography variant="subtitle2">Nom bénéficiaire</Typography>
                    <Typography variant="body1">{selected!.nom_beneficiaire}</Typography>
                  </Box>
                )}
                ...
                {(selected?.mois || null) && (
                  <Box>
                    ...
                    <Typography variant="body1">{selected!.mois}</Typography>
                  </Box>
                )}

                {(selected?.nom_vehicule || null) && (
                  <Box>
                    <Typography variant="subtitle2">Nom véhicule</Typography>
                    <Typography variant="body1">{selected!.nom_vehicule}</Typography>
                  </Box>
                )}

                {(selected?.nom_commercial || null) && (
                  <Box>
                    <Typography variant="subtitle2">Nom Commercial</Typography>
                    <Typography variant="body1">{selected!.nom_commercial}</Typography>
                  </Box>
                )}
                ...
                {(selected?.provenance || null) && (
                  <Box>
                    ...
                    <Typography variant="body1">{selected!.provenance}</Typography>
                  </Box>
                )}

                {(selected?.nom_produit || null) && (
                  <Box>
                    <Typography variant="subtitle2">Nom du produit</Typography>
                    <Typography variant="body1">{selected!.nom_produit}</Typography>
                  </Box>
                )}

                {(typeof selected?.quantite_kg === 'number' && selected.quantite_kg > 0) && (
                  <Box>
                    <Typography variant="subtitle2">Quantité (kg)</Typography>
                    <Typography variant="body1">{selected!.quantite_kg}</Typography>
                  </Box>
                )}

                {(selected?.dossier_importation || null) && (
                  <Box>
                    <Typography variant="subtitle2">Dossier d’importation</Typography>
                    <Typography variant="body1">{selected!.dossier_importation}</Typography>
                  </Box>
                )}

                {(selected?.fournisseur || null) && (
                  <Box>
                    <Typography variant="subtitle2">Fournisseur</Typography>
                    <Typography variant="body1">{selected!.fournisseur}</Typography>
                  </Box>
                )}
                ...
                <Box>
                  ...
                  <Typography variant="body1">
                    {selected?.montant_ttc != null ? `${formatDzd(selected.montant_ttc)} DZD` : '-'}
                  </Typography>
                </Box>

                <Box>
                  <Typography variant="subtitle2">Statut</Typography>
                  <Typography variant="body1">{selected?.statut ?? '-'}</Typography>
                </Box>

                {(selected?.motif_rejet || null) && (
                  <Box>
                    <Typography variant="subtitle2">Motif du rejet</Typography>
                    <Typography
                      variant="body1"
                      sx={{ overflowWrap: 'anywhere', wordBreak: 'break-word' }}
                    >
      ...
      <Container maxWidth={false} sx={{ px: { xs: 2, sm: 4, md: 6 }, mt: 4, mb: 6 }}>
              ...
              <TableBody>
                {depensesTicketsReglement.length === 0 ? (
                  ...
                  depensesTicketsReglement.map((t) => {
                    const mode = (t.mode_reglement ?? '').toString().trim().toLowerCase();
                    const canMarkReglee = mode === 'espèce' || mode === 'espece';
                    return (
                      <TableRow
                        key={t.id}
                        hover
                      ...
                      >
                        ...
                        <TableCell align="center" sx={{ whiteSpace: 'nowrap' }}>
                          <Button
                            size="small"
                            variant="contained"
                            disabled={!canMarkReglee || isActionSubmitting || !!t.reglee_at}
                            onClick={(e) => {
                              e.stopPropagation();
